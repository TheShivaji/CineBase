...
export const Header = () => {
  ...
  const activeClass = "text-base block py-2 px-3 text-white bg-blue-700 rounded md:bg-transparent md:text-blue-700 md:p-0 dark:text-white dark:hover:bg-gray-700";
...
return Navigate(`/search?q=${queryTerm}`)
  ...
  return (
    ...
    <nav className="bg-white border-b border-gray-200 fixed w-full z-20 top-0 start-0 dark:bg-gray-900 dark:border-gray-700">
      <div className="max-w-7xl flex flex-wrap items-center justify-between mx-auto p-4">

        <a href="/"  className="flex items-center space-x-3 rtl:space-x-reverse">
          <img src={ownlogo} className="h-7" alt="Flowbite Logo" />
          {/* 2. Logo text ko dark mode mein white kiya */}
          <span className="self-center text-xl font-semibold whitespace-nowrap text-gray-900 dark:text-white">CineBase</span>
        </a>

        <div className="flex items-center md:order-2">
          {/* 3. Dark Mode Toggle Button: Isme already dark classes thi, bas thoda sa hover fix kiya */}
          <button onClick={() => setdarkMode(!darkMode)} data-tooltip-target="navbar-search-example-toggle-dark-mode-tooltip" type="button" data-toggle-dark="light" className="flex items-center p-2 mr-2 text-xs font-medium text-gray-700 bg-white rounded-lg border border-gray-200 toggle-dark-state-example hover:bg-gray-100 hover:text-blue-700 focus:z-10 focus:ring-2 focus:ring-gray-300 focus:outline-none dark:text-gray-400 dark:bg-gray-800 dark:border-gray-600 dark:hover:text-white dark:hover:bg-gray-700">
            {darkMode ? (
          ...
          <button type="button" data-collapse-toggle="navbar-search" aria-controls="navbar-search" aria-expanded="false" className="md:hidden text-gray-500 hover:bg-gray-100 focus:outline-none focus:ring-4 focus:ring-gray-200 rounded-lg text-sm p-2.5 me-1 dark:text-gray-400 dark:hover:bg-gray-700 dark:focus:ring-gray-700">
        ...
        <div className={`items-center justify-between ${hidden ? 'hidden' : ''} w-full md:flex md:w-auto md:order-1`} id="navbar-search">
          ...
          <ul className="flex flex-col p-4 md:p-0 mt-4 font-medium border border-gray-100 rounded-lg bg-gray-50 md:space-x-8 rtl:space-x-reverse md:flex-row md:mt-0 md:border-0 md:bg-white  dark:bg-gray-800 md:dark:bg-gray-900 dark:border-gray-700  ">
            <li>
              <NavLink to="/" className={({ isActive }) => isActive ? activeClass : inactiveClass} onClick={() => sethidden(true)}>Home</NavLink>
            </li>
            <li>
              <NavLink to="movies/popular" className={({ isActive }) => isActive ? activeClass : inactiveClass} onClick={() => sethidden(true)}>Popular</NavLink>
            </li>
            <li>
              <NavLink to="movies/top" className={({ isActive }) => isActive ? activeClass : inactiveClass} onClick={() => sethidden(true)}>Top</NavLink>
            </li>
            <li>
